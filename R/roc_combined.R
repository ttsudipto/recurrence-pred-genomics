...
get_plot <- function(ds, title1, title2) {
  ...
  original_plot <- ggplot(plot_data_original, mapping=aes(x=x_original, y=y_original, group=ml_original)) +
    ...
    ggtitle(paste0(title1, " Original")) + 
    xlab(xlab) + ylab(ylab) +
...
plot_1 <- get_plot("dataset1", "(a) Dataset 1", "(b) Dataset 1")
# # svg(paste0("output/dataset1/plots/roc_plot_optimal.svg"), width=13, height=6)
# plot_1$original | plot_1$smote
# # dev.off()

plot_2 <- get_plot("dataset2", "(c) Dataset 2", "(d) Dataset 2")
# # svg(paste0("output/dataset2/plots/roc_plot_optimal.svg"), width=13, height=6)
# plot_2$original | plot_2$smote
# # dev.off()

plot_3 <- get_plot("dataset3", "(e) Dataset 3", "(f) Dataset 3")
# # svg(paste0("output/dataset3/plots/roc_plot_optimal.svg"), width=13, height=6)
# plot_3$original | plot_3$smote
# # dev.off()

svg(filename='output/roc_all.svg', height=18, width=13)
(plot_1$original | plot_1$smote) / (plot_2$original | plot_2$smote) / (plot_3$original | plot_3$smote)
dev.off()
